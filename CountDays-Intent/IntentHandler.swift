...
//  CountDays-Intent
//
//  Created by shoichiyamazaki on 2023/09/25.
//
...
class IntentHandler: INExtension, INSendMessageIntentHandling, INSearchForMessagesIntentHandling, INSetMessageAttributeIntentHandling, ConfigurationIntentHandling {
    func provideEventParaOptionsCollection(for intent: ConfigurationIntent, with completion: @escaping (INObjectCollection<EventItem>?, Error?) -> Void) {
        var eventItems: [EventItem] = []
        
        let _ = RealmViewModel().events.enumerated().map {
            let eventItem = EventItem(identifier: $0.offset.description, display: $0.element.title)
            eventItems.append(eventItem)
        }
        
        /// 空のときの処理？
        let all = INObjectCollection(items: eventItems)
...
//            "ccc"
//        ]
//        let allName = INObjectCollection(items: names)
//        completion(allName, nil)
//    }
